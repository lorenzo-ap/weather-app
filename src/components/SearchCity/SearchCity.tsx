...
const SearchCity = ({ setWeatherData }: SearchCityProps) => {
    const [inputValue, setInputValue] = useState('');
    const [suggestedCities, setSuggestedCities] = useState<Suggestion[]>([]);
    const [APIdata, setAPIData] = useState([]);
    const debouncedHandleInputChange = useRef<(event: ChangeEvent<HTMLInputElement>) => void>();
    ...
    useEffect(() => {
        debouncedHandleInputChange.current = debounce(handleInputChange, 300);
    }, []);
    ...
    const searchCity = () => {
        inputValue !== '' &&
            ...
            axios
                .get(
                    `https://api.weatherapi.com/v1/forecast.json?key=231b8f0b79954d95bc494854230905&q=${inputValue}&days=7`
                )
            ...
            <input
                ...
                onChange={(e) => {
                    ...
                    if (debouncedHandleInputChange.current) {
                        debouncedHandleInputChange.current(e);
                    }
                }}
                ...
                <div className="absolute top-[50px] left-0 w-full backdrop-blur-md rounded-3xl shadow-xl">
                    <div className="w-full bg-[rgb(248,250,252,.65)] dark:bg-[rgb(31,42,58,.75)] rounded-3xl py-2 px-4">
                        {suggestedCities.map((suggestion: City) => (
                            <div
                                onClick={setCurrentSuggestedCity}
                                key={suggestion.name}
                                className="cursor-pointer py-0.5 hover:opacity-60 transition-opacity">
                                {suggestion.name}
                            </div>
                        ))}
                    </div>
